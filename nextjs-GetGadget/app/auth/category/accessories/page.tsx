...
function SkeletonCard() {
  return (
    <div className="relative bg-white rounded-xl border border-emerald-100 p-4 overflow-hidden shadow-sm">
      <div className="absolute inset-0 -translate-x-full animate-[shimmer_1.5s_infinite] bg-gradient-to-r from-transparent via-emerald-100/50 to-transparent" />
      <div className="h-48 sm:h-60 w-full bg-emerald-100 rounded-lg mb-4" />
      <div className="h-4 bg-emerald-100 rounded-full w-3/4 mx-auto mb-3" />
      <div className="h-8 bg-emerald-100 rounded w-full mb-3" />
      <div className="h-4 bg-emerald-100 rounded-full w-1/3 mx-auto mb-4" />
      <div className="h-9 bg-emerald-100 rounded-md w-full" />
    </div>
  );
}
...
export default function AccessoriesPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await axios.get("/api/admin/products?category=accessories");
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-emerald-50/50 via-white to-teal-50/50">
      <style>{`
        @keyframes shimmer {
        ...
        .gadget-icon {
          animation: gadget-pulse 2s ease-in-out infinite;
        }
      `}</style>
      ...
      <div className="relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-emerald-100/40 via-white to-teal-100/30" />
        <div className="absolute -top-32 -right-32 h-[400px] w-[400px] rounded-full bg-gradient-to-br from-emerald-200/30 to-emerald-300/20 blur-[120px]" />
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="flex justify-center lg:justify-end">
              <div className="relative">
                <div className="absolute -inset-4 rounded-[32px] bg-gradient-to-br from-emerald-200/40 to-teal-200/40 blur-xl" />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14">
        <div className="mb-10 flex items-center justify-between">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-emerald-100/80 border border-emerald-200 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-emerald-600 mb-4">
              <Sparkles className="h-3 w-3" />
              Collection
            </div>
            <h2 className="text-3xl font-bold text-gray-900">Gadget Collection</h2>
            <p className="text-gray-400 text-sm mt-1">Browse our tech accessories</p>
          </div>
          {!loading && products.length > 0 && (
            <span className="hidden sm:inline-flex items-center gap-1.5 text-sm text-emerald-500 font-medium">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 gap-4 text-center">
            <div className="flex h-20 w-20 items-center justify-center rounded-full bg-emerald-100 mb-2">
              <Smartphone className="w-10 h-10 text-emerald-400" />
            </div>
            <p className="text-2xl font-bold text-gray-400">No gadgets yet</p>
            <p className="text-gray-400 text-sm">Check back soon for new tech accessories.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {products.map((product, index) => {
              ...
              return (
                <div
                  key={product._id}
                  className="relative group bg-white rounded-2xl border border-emerald-100 p-4 shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lg hover:shadow-emerald-200/30"
                >
                  <button
                  ...
                  <div className="relative mb-4">
                    <div className="h-48 sm:h-56 w-full overflow-hidden rounded-xl bg-emerald-50">
                      {imgSrc ? (
                        <img
                          src={imgSrc}
                          alt={product.name}
                      ...
                      <div className="absolute top-2 left-2 inline-flex items-center gap-1 rounded-full bg-emerald-500/90 px-2.5 py-0.5 text-[10px] font-bold text-white shadow-lg backdrop-blur-sm">
                        <Smartphone className="h-3 w-3 fill-white" />
                        GADGET
                      </div>
                    </div>
                  </div>
                  ...
                  <p className="text-center mt-2 font-bold text-emerald-600">Rs. {product.price}</p>

                  <button
                    onClick={() => addToCart(product._id)}
                    className="mt-4 w-full bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white py-2.5 rounded-full text-sm font-semibold transition-all duration-200 shadow-lg shadow-emerald-500/25 hover:shadow-emerald-600/30 cursor-pointer"
                  >
                    Add to Cart
                  </button>
                </div>
